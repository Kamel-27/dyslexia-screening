...
function getVisualCueClassName(visualCue: string, isLightTheme: boolean) {
  ...
  if (isSentenceLike) {
    return `w-full max-w-5xl rounded-sm border ${baseTheme} px-5 py-4 text-center text-base leading-relaxed tracking-normal sm:px-8 sm:py-5 sm:text-xl`;
  }

  if (isArabicText) {
    return `min-w-65 rounded-sm border ${baseTheme} px-8 py-5 text-center text-4xl tracking-normal sm:min-w-105 sm:text-5xl`;
  }

  return `min-w-65 rounded-sm border ${baseTheme} px-8 py-5 text-center text-4xl tracking-[0.3em] sm:min-w-105 sm:text-5xl`;
}
...
export default function TestPage() {
  ...
  const completeTypedRoundRef = useRef<((currentQuestion: Question, submittedText: string) => void) | null>(null);
  ...
  const playBackgroundMusic = useCallback(() => {
    const audio = backgroundMusicRef.current;
    if (!audio || isBackgroundMusicMuted || isLightTheme) {
      return;
    }
  ...
  }, [isBackgroundMusicMuted, playBackgroundMusic, isLightTheme]);
  ...
  async function finishSession() {
    ...
    router.push(`/gamified-test/result/${sessionId}?theme=${isLightTheme ? 'light' : 'dark'}`);
  }
  ...
  completeTypedRoundRef.current = completeTypedRound;
  ...
  useEffect(() => {
    ...
    // DO NOT start the question or voice playback if instructions are still open!
    if (isLightTheme && showPreTestInstructions) {
      return;
    }

    // Light theme does not wait for isSpeechReady or block the user with a setup screen
    if (!isSpeechReady && !isLightTheme) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setTimeLeft((previous) => {
        if (previous <= 1) {
          if (isTypedRecallMode) {
            completeTypedRoundRef.current?.(questionRef.current, typedInputRef.current);
          } else {
            setPhase("summary");
          }
          return 0;
  ...
  if (isLightTheme && showPreTestInstructions) {
    return (
      <main
        className="relative h-dvh w-full overflow-hidden bg-[#fafbf9] flex items-center justify-center p-4 sm:p-6"
        dir={isArabicExam ? "rtl" : "ltr"}
      >
        <div aria-hidden className="absolute inset-0" style={{ background: "radial-gradient(circle at left center, #f0fdf4 0%, #fafbf9 100%)" }} />

        <div className="relative z-10 w-full max-w-lg bg-white rounded-3xl border border-emerald-100 shadow-sm p-6 sm:p-8 text-center animate-fade-in">
          <div className="w-12 h-12 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center justify-center mx-auto mb-4">
            <Gamepad2 className="w-6 h-6 text-emerald-800" />
          </div>
          
          <h1 className="text-xl font-black text-emerald-950">
            {isArabicExam ? "تعليمات التقييم" : "Screening Assessment"}
          </h1>
          <p className="text-xs text-slate-500 mt-2 leading-relaxed">
            {isArabicExam 
              ? "مرحبًا بك في ليكسورا! سنبدأ اختبارًا قصيرًا وممتعًا لتقييم المهارات اللغوية والبصرية. يرجى اتباع التعليمات التالية:"
              : "Welcome to Lexora! We are about to begin a short, fun screening to evaluate language and visual skills. Please follow these simple instructions:"}
          </p>

          <div className="my-6 text-left space-y-3.5 text-xs text-slate-600 bg-emerald-50/20 border border-emerald-100/50 rounded-2xl p-4 sm:p-5">
            <div className="flex gap-2.5">
              <span className="text-base">🔊</span>
              <div>
                <strong className="text-emerald-950 font-bold block text-[12px]">{isArabicExam ? "الاستماع التلقائي" : "Listen Automatically"}</strong>
                <span className="text-slate-500 text-[11px] leading-relaxed block mt-0.5">
                  {isArabicExam 
                    ? "ستُقرأ جميع الأسئلة والتعليمات بصوت عالٍ وتلقائيًا في الخلفية."
                    : "Instructions and tasks will play out loud automatically in the background."}
                </span>
              </div>
            </div>
            ...
            <div className="flex gap-2.5">
              <span className="text-base">⚡</span>
              <div>
                <strong className="text-emerald-950 font-bold block text-[12px]">{isArabicExam ? "إجابة سريعة ودقيقة" : "Answer Quickly"}</strong>
                <span className="text-slate-500 text-[11px] leading-relaxed block mt-0.5">
                  {isArabicExam 
                    ? "اضغط على الحروف أو رتب الكلمات بشكل صحيح بأسرع ما يمكن."
                    : "Tap or click the correct tiles as fast and accurately as possible."}
                </span>
              </div>
            </div>
            ...
            <div className="flex gap-2.5">
              ...
              <div>
                <strong className="text-emerald-950 font-bold block text-[12px]">{isArabicExam ? "مؤشر الوقت" : "Time Progress"}</strong>
                <span className="text-slate-500 text-[11px] leading-relaxed block mt-0.5">
                  {isArabicExam 
                    ? "انتبه للمؤشر الدائري في الزاوية العلوية لمعرفة الوقت المتبقي."
                    : "A circle indicator in the top corner shows your time remaining."}
                </span>
              </div>
            </div>
          </div>
          ...
          <button
            type="button"
            onClick={() => {
              // Browser Speech Synthesis activation unlock
            ...
            className="w-full py-3.5 rounded-2xl text-sm font-bold bg-emerald-700 hover:bg-emerald-800 text-white transition shadow-md shadow-emerald-800/10 hover:shadow-lg"
          >
            {isArabicExam ? "ابدأ التقييم" : "Start Assessment"}
          </button>
        </div>
      </main>
    );
  ...
  if (phase === "voice") {
    return (
      ...
      >
        ...
        {isLightTheme && <div aria-hidden className="absolute inset-0" style={{ background: "radial-gradient(circle at left center, #f0fdf4 0%, #fafbf9 100%)" }} />}
      </main>
    );
  }
        ...
        <div className="relative z-10 mx-auto flex h-full w-full max-w-3xl items-center justify-center px-6 py-10">
          <div className={`w-full max-w-md rounded-2xl p-8 text-center ${
              !isLightTheme
                ...
                : "border border-emerald-100 bg-white shadow-[0_4px_24px_rgba(0,0,0,0.04)]"
            }`}>
            <p className={`text-sm font-medium ${ !isLightTheme ? "text-slate-500" : "text-slate-500" }`}>
              {question.title} {copy.completeLabel}
            </p>
            <h1 className={`mt-4 text-3xl font-semibold ${ !isLightTheme ? "text-slate-800" : "text-emerald-800" }`}>
              {copy.pointsPrefix} {score} {copy.pointsSuffix}
            </h1>
            <p className={`mt-2 ${ !isLightTheme ? "text-slate-500" : "text-slate-500" }`}>
            ...
            <button
              ...
              className={`mt-8 inline-flex w-full items-center justify-center rounded-xl px-4 py-3 font-semibold transition-all disabled:cursor-not-allowed disabled:opacity-60 ${
                !isLightTheme
                  ...
                  : "bg-emerald-700 border border-emerald-700 text-white hover:bg-emerald-850 shadow-sm"
              }`}
            >
              {phase === "finishing"
                ? copy.finishing
        ...
        <div className="absolute left-4 top-4 z-20 flex items-center gap-2 sm:left-7 sm:top-6">
          <button
            ...
            className={`rounded-lg px-3 py-1.5 text-sm font-semibold ${
              !isLightTheme
                ...
                : "border border-emerald-100 bg-[#fbfcfb] text-emerald-800 hover:bg-emerald-50 shadow-sm"
            }`}
          >
            {copy.skip}
          </button>
          {!isLightTheme && (
            <button
              ...
              className="rounded-lg p-2 transition-colors border border-white/25 bg-white/90 text-slate-700 hover:bg-white"
            >
              {isBackgroundMusicMuted
        ...
        <div
          ...
          style={{
            background: isLightTheme
              ? `conic-gradient(#047857 ${timeProgress}%, #d1fae5 ${timeProgress}% 100%)`
              : `conic-gradient(#18a7d3 ${timeProgress}%, #9dd2ea ${timeProgress}% 100%)`,
          }}
        >
          <div className={`flex h-full w-full items-center justify-center rounded-full ${
            isLightTheme ? "bg-white text-emerald-800 shadow-sm border border-emerald-50" : "bg-white text-slate-700"
          }`}>
      ...
      <div className="relative z-10 flex h-full w-full items-center justify-center px-3 py-3 sm:px-6 sm:py-6">
        <div className="flex flex-col items-center gap-7 sm:gap-9">
          ...
          {interactionType === "typedSequenceRecall" ? (
            <>
              <div className={`rounded-xl px-8 py-5 text-center sm:min-w-105 ${
                !isLightTheme
                  ...
                  : "border border-indigo-100 bg-white shadow-sm text-slate-800"
              }`}>
                <div className={`text-xs font-semibold uppercase tracking-[0.12em] ${ !isLightTheme ? "text-slate-400" : "text-slate-400" }`}>
                ...
                {isVoicePlaying && typedRoundStage === "cue" ? (
                  <p className={`mt-2 text-sm ${ !isLightTheme ? "text-slate-400" : "text-slate-400" }`}>
            ...
            <>
              <div className={`rounded-xl border px-6 py-4 text-center sm:px-8 sm:py-5 ${
                isLightTheme ? 'border-emerald-100 bg-white text-slate-800 shadow-sm' : 'border-sky-200 bg-white text-slate-700'
              }`}>
                    ...
                    <button
                      ...
                      className={`relative border-b-2 px-0.5 font-semibold leading-none transition ${
                        pendingLetterReplacementIndex === letterIndex
                          ? isLightTheme ? "border-emerald-500 text-emerald-700" : "border-sky-500 text-sky-700"
                          : isLightTheme ? "border-transparent text-slate-800 hover:border-emerald-350" : "border-transparent text-slate-700 hover:border-sky-300"
                      }`}
                      style={{
                ...
                >
                  {gridCells.map((cell, cellIndex) => (
                    <button
                      ...
                      className={`rounded-sm border px-1 font-bold leading-tight transition ${
                          isLightTheme
            ...
            <>
              {selectedArrangementTokens.length > 0 && (
                <div className={`rounded-xl border px-8 py-4 text-center text-3xl tracking-widest sm:text-4xl ${
                  isLightTheme ? 'border-emerald-100 bg-emerald-50 text-emerald-800 shadow-sm' : 'border-sky-200 bg-sky-50 text-slate-700'
                }`}>
                ...
                {gridCells.map((cell, cellIndex) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => handleAnswerClick(cell, cellIndex)}
                      disabled={isSelected}
                      className={`rounded-xl border-2 px-1 font-semibold leading-tight transition ${
                        isSelected
                          ? isLightTheme
                          ...
                          : isLightTheme
                            ? "border-emerald-100 bg-white text-slate-800 hover:border-emerald-350 hover:bg-emerald-50"
            ...
            <div className={`w-full max-w-5xl rounded-xl border px-5 py-4 text-center leading-relaxed sm:px-8 sm:py-5 ${
              isLightTheme ? 'border-emerald-100 bg-white text-slate-800 shadow-sm' : 'border-sky-200 bg-white text-slate-600'
            }`}>
              {sentenceTokens.map((token, tokenIndex) => (
                <button
                  ...
                  className={`mx-1 my-1 inline rounded-lg px-2 py-1 text-2xl transition ${
                    isLightTheme ? 'text-slate-800 hover:bg-emerald-50 hover:text-emerald-800' : 'text-slate-600 hover:bg-sky-50'
                  }`}
                >
                  {token}
                </button>
              ...
              {gridCells.map((cell, cellIndex) => (
                <button
                  ...
                  className={
                    isClassicGridMode
                      ? isLightTheme
                        ? "rounded-2xl border-2 border-emerald-100 bg-white px-1 text-center font-bold leading-none text-slate-800 shadow-sm transition active:scale-95 hover:border-emerald-350 hover:bg-emerald-50"
                      ...
                      : isLightTheme
                      ? "rounded-xl border-2 border-emerald-100 bg-white px-1 text-center font-bold leading-tight text-slate-800 transition hover:border-emerald-350 hover:bg-emerald-50"
